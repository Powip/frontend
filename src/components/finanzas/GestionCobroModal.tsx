...
const mockPedidos = [
  ...
  },
  {
    ...
    distrito: "Cusco",
    warningText: "Shalom cobra retorno (tarifa variable por ciudad)",
    adelanto: 0,
    monto: 89.0,
    estado: "Retorno",
    entregado: false,
...
export function GestionCobroModal({
  ...
  const pedidosEnAgencia = mockPedidos.filter((p) => p.estado === "En agencia");

  const pedidosFiltrados =
    pedidosFiltro === "Todos"
      ? mockPedidos
      : mockPedidos.filter((p) => p.estado === pedidosFiltro);
      ...
      <DialogContent className="min-w-7xl p-0 overflow-y-hidden rounded-xl h-[95vh] flex flex-col shadow-2xl bg-background">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[13px] text-muted-foreground dark:text-slate-400 mt-0.5">
                {data.courier} · Lima Sur · {data.pedidos} pedidos · {data.date}
              </p>
            </div>
          </div>
            ...
            <div className="bg-teal-50 dark:bg-teal-950/40 border border-teal-200 dark:border-teal-900/60 rounded-xl p-5 flex justify-between items-center shadow-sm">
              <div>
                <div className="flex items-center gap-2 text-teal-800 dark:text-teal-400 font-semibold mb-1 text-[13px]">
                  <span className="text-base leading-none">💸</span> El negocio
                  le debe pagar el flete al courier
                </div>
                <div className="text-2xl font-bold text-teal-700 dark:text-teal-300">
                  S/ 16.00
                </div>
                <div className="text-xs text-teal-600/80 dark:text-teal-400/80 mt-1">
                  2 entregas exitosas × S/8.00 · {data.courier} · Lima Sur
                </div>
              </div>
              <Button className="bg-teal-700 dark:bg-teal-600 hover:bg-teal-800 dark:hover:bg-teal-700 text-white shadow-sm font-semibold">
          ...
          {isTipoA && (
            <div className="bg-amber-50/40 dark:bg-amber-950/20 p-4 -mx-6 px-6 border-y border-amber-100/50 dark:border-amber-900/30 space-y-3">
              <h3 className="text-[11px] font-bold text-amber-700 dark:text-amber-400 uppercase tracking-wider">
                ¿QUÉ COBRA {data.courier.toUpperCase()} AL CLIENTE?
              </h3>
                  ...
                  <div className="flex items-start gap-3 flex-1">
                    ...
                    <div>
                      <p className="font-bold text-[13px] text-green-900 dark:text-green-300">
                        Clave de recojo {data.courier}
                      </p>
                      <p className="text-[12px] text-green-700 dark:text-green-400 mt-0.5 leading-snug">
                        El cliente pagó al negocio. Envíale la clave de recojo.
                        Sin esta clave, {data.courier} no entrega el paquete.
                      </p>
                    </div>
                  </div>
                  <Button
            ...
            >
              <div className="flex items-start gap-3 flex-1">
                ...
                <div>
                  <p className="font-bold text-[13px] text-blue-900 dark:text-blue-300">
                    ...
                    {data.courier} esperando recojo
                  </p>
                  <p className="text-[12px] text-blue-700 dark:text-blue-400 mt-0.5 leading-snug">
                    Hasta 30 días para recoger. Si no recogen: {data.courier}{" "}
                    cobra retorno (tarifa variable por ciudad).
                  </p>
                </div>
              </div>
              <div className="flex gap-2 sm:shrink-0">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  className="border-blue-300 dark:border-blue-800 text-blue-700 dark:text-blue-400 bg-white dark:bg-slate-900 hover:bg-blue-50 dark:hover:bg-blue-900/30 focus:ring-2 focus:ring-blue-300 dark:focus:ring-blue-800 font-semibold gap-1.5"
                >
                  <MessageCircle className="h-3.5 w-3.5" />
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-1.5">
                <Badge
                  variant={pedidosFiltro === "Todos" ? "secondary" : "outline"}
                  className={`rounded-full text-[11px] px-3 py-0.5 font-medium cursor-pointer ${pedidosFiltro === "Todos" ? "bg-teal-700 dark:bg-teal-600 hover:bg-teal-800 dark:hover:bg-teal-700 text-white border-0 shadow-sm" : "text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800"}`}
                  onClick={() => setPedidosFiltro("Todos")}
                >
                  Todos
                </Badge>
                <Badge
                  variant={
                    pedidosFiltro === "En agencia" ? "secondary" : "outline"
                  }
                  className={`rounded-full text-[11px] px-3 py-0.5 font-medium cursor-pointer ${pedidosFiltro === "En agencia" ? "bg-teal-700 dark:bg-teal-600 hover:bg-teal-800 dark:hover:bg-teal-700 text-white border-0 shadow-sm" : "text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800"}`}
                  onClick={() => setPedidosFiltro("En agencia")}
                >
                  En agencia
                </Badge>
                <Badge
                  variant={
                    pedidosFiltro === "Cobrado" ? "secondary" : "outline"
                  }
                  className={`rounded-full text-[11px] px-3 py-0.5 font-medium cursor-pointer ${pedidosFiltro === "Cobrado" ? "bg-teal-700 dark:bg-teal-600 hover:bg-teal-800 dark:hover:bg-teal-700 text-white border-0 shadow-sm" : "text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800"}`}
                  onClick={() => setPedidosFiltro("Cobrado")}
                >
                  Cobrados
                </Badge>
                <Badge
                  variant={
                    pedidosFiltro === "Rechazado" ? "secondary" : "outline"
                  }
                  className={`rounded-full text-[11px] px-3 py-0.5 font-medium cursor-pointer ${pedidosFiltro === "Rechazado" ? "bg-teal-700 dark:bg-teal-600 hover:bg-teal-800 dark:hover:bg-teal-700 text-white border-0 shadow-sm" : "text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800"}`}
                  onClick={() => setPedidosFiltro("Rechazado")}
                >
                  Rechazados
                </Badge>
                <Badge
                  variant={
                    pedidosFiltro === "Retorno" ? "secondary" : "outline"
                  }
                  className={`rounded-full text-[11px] px-3 py-0.5 font-medium cursor-pointer ${pedidosFiltro === "Retorno" ? "bg-teal-700 dark:bg-teal-600 hover:bg-teal-800 dark:hover:bg-teal-700 text-white border-0 shadow-sm" : "text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800"}`}
                  onClick={() => setPedidosFiltro("Retorno")}
                >
                  Retorno
                </Badge>
                <Badge
                  variant={
                    pedidosFiltro === "En envío" ? "secondary" : "outline"
                  }
                  className={`rounded-full text-[11px] px-3 py-0.5 font-medium cursor-pointer ${pedidosFiltro === "En envío" ? "bg-teal-700 dark:bg-teal-600 hover:bg-teal-800 dark:hover:bg-teal-700 text-white border-0 shadow-sm" : "text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800"}`}
                  onClick={() => setPedidosFiltro("En envío")}
                >
                  En envío
                </Badge>
              </div>
            ...
            <div className="space-y-2.5">
              {pedidosFiltrados.length === 0 ? (
                ...
                pedidosFiltrados.map((pedido) => {
                  const CheckboxUI = () => (
                    <div
                      className={`w-[18px] h-[18px] rounded-[4px] flex items-center justify-center shrink-0 shadow-sm transition-colors ${pedido.checked ? "bg-emerald-500 text-white border-none" : "bg-slate-50/50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700"}`}
                    >
                      {pedido.checked && (
                            ...
                            {pedido.dias && (
                              <div className="flex items-center gap-2">
                                <div className="h-1 w-12 bg-slate-200 rounded-full overflow-hidden">
                                  <div
                                    className={`h-full ${pedido.diasPorcentaje && pedido.diasPorcentaje > 60 ? "bg-red-500" : "bg-amber-500"}`}
                                    style={{
                                      width: `${pedido.diasPorcentaje}%`,
                                    }}
                                  ></div>
                                </div>
                                <span
                                  className={`text-[10px] font-bold leading-none ${pedido.diasPorcentaje && pedido.diasPorcentaje > 60 ? "text-red-500 dark:text-red-400" : "text-amber-500 dark:text-amber-400"}`}
                                >
                                  {pedido.dias}
                                </span>
                              </div>
          ...
          {isTipoB && (
            <div className="bg-blue-50/60 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900/50 rounded-xl p-4 space-y-3">
              <div className="flex items-center gap-2 text-blue-700 dark:text-blue-400 font-semibold text-[13px]">
                <span className="text-base leading-none">💸</span> Flete a pagar
                por el negocio al courier
              </div>
              <div className="bg-white dark:bg-slate-900 rounded-lg border border-blue-50 dark:border-slate-800 p-3 flex justify-between items-center shadow-sm">
                <div>
                  <div className="text-[12px] text-blue-700/70 dark:text-blue-400/70 mb-0.5">
                    Entregas exitosas × tarifa Lima Sur
                  </div>
                  <div className="text-2xl font-bold text-blue-800 dark:text-blue-300">
                    S/ 16.00
                  </div>
                </div>
                <Button className="bg-blue-600 dark:bg-blue-700 hover:bg-blue-700 dark:hover:bg-blue-600 text-white font-semibold">
                  Registrar pago de flete
                </Button>
              </div>
            </div>
          ...
          <div className="border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-slate-900 overflow-hidden shadow-sm">
            <div className="p-3.5 px-4 border-b flex justify-between items-center cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
              <div className="flex items-center gap-2 font-semibold text-[13px] text-slate-700 dark:text-slate-300">
                <span className="text-base leading-none">📊</span> Desglose de
                costos del courier
              </div>
            ...
            <div className="p-3.5 px-4 flex justify-between items-center bg-amber-50/30 dark:bg-amber-950/30">
              <div className="font-semibold text-amber-600 dark:text-amber-400 text-[13px]">
                Saldo pendiente (ya pagó S/ 1,000)
              </div>
          ...
          <div className="space-y-4 pt-2">
            ...
            {isTipoB && (
              <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900/50 rounded-lg p-3 text-[13px] text-blue-800 dark:text-blue-300">
                <span className="font-bold">
                  Negocio paga el flete al courier.
                </span>{" "}
                El cliente pagó al negocio. Registra cuánto le debes al courier
                por las entregas.
              </div>
            )}
            {isTipoC && (
              <div className="bg-purple-50/80 dark:bg-purple-950/30 border border-purple-100 dark:border-purple-900/50 rounded-lg p-3 text-[13px] text-purple-800 dark:text-purple-400">
                <span className="font-bold">Olva prepagado.</span> El envío ya
                fue pagado al registrar. Solo confirma los costos operativos.
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <Input
                    defaultValue="19/04/2026"
                    className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 pl-3 pr-10 h-10 shadow-sm"
                  />
                  <Calendar className="absolute right-3 top-2.5 h-4 w-4 text-slate-400 dark:text-slate-500" />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="border border-dashed border-slate-300 dark:border-slate-700 rounded-lg p-3 text-center bg-white dark:bg-slate-900 flex items-center justify-center gap-2 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                placeholder="Ej: Shalom rindió en 2 partes · Falta confirmar pedidos de Iquitos · Descuento por almacenaje aplicado · Courier llegó tarde a liquidar..."
            ...
            <div className="bg-emerald-50/50 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/50 text-emerald-700 dark:text-emerald-400 text-[12px] rounded-lg p-3 text-center">
              <span className="font-bold">Al registrar:</span> Saldo de la guía
              se actualiza · Si llega a S/0.00 queda liquidada · Observación
              guardada en el historial del pago
            </div>
          </div>
        </div>
        ...
        <DialogFooter className="p-4 px-6 border-t bg-white dark:bg-slate-900 flex justify-between items-center sm:justify-between shrink-0">
          ...
          <div className="flex gap-3">
            ...
            <Button className="bg-teal-700 dark:bg-teal-600 hover:bg-teal-800 dark:hover:bg-teal-700 text-white gap-2 px-6 font-semibold h-10 shadow-md">
