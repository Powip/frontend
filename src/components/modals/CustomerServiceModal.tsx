...
const PRINT_DELIVERY_TYPE_LABEL: Record<string, string> = {
...
export default function CustomerServiceModal({
  ...
  const handlePrint = async () => {
    ...
    // rastreo" más abajo) — antes codificaba el orderId crudo (un UUID), que
    ...
    const printContent = `
      ...
      <body>
        <div class="company-header">
          <div class="logo-box">
            ${auth?.company?.logoUrl ? `<img src="${auth.company.logoUrl}" alt="Logo">` : "LOGO"}
          </div>
          <div>
            <div class="company-name">${auth?.company?.name ?? "-"}</div>
            ${auth?.company?.cuit ? `<div class="company-meta">RUC ${auth.company.cuit}</div>` : ""}
          </div>
        </div>
