...
import { getIntegraciones, toggleIntegracion, reconectarIntegracion } from "@/services/superadmin/integracionesService";
import { Card, CardContent } from "@/components/ui/card";
import { Switch } from "@/components/ui/switch";
import { Button } from "@/components/ui/button";
import { StatusBadge, BadgeTone, TableSkeleton, EmptyBlock } from "@/components/superadmin/shared";
...
export function IntegracionesGrid() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["superadmin", "integraciones", "list"],
    queryFn: getIntegraciones,
  });

  const { mutate: toggle } = useMutation({
    mutationFn: toggleIntegracion,
  ...
  const { mutate: reconectar } = useMutation({
    mutationFn: reconectarIntegracion,
    onSuccess: (integracion) => {
      queryClient.invalidateQueries({ queryKey: ["superadmin", "integraciones"] });
        ...
        <Card key={i.id} className="shadow-sm">
          <CardContent className="p-4">
            <div className="flex items-start justify-between gap-2">
              ...
              <Switch checked={i.activa} onCheckedChange={() => toggle(i.id)} />
            </div>
            ...
            {i.estado !== "operativo" && (
              <Button size="sm" variant="outline" className="mt-3 w-full gap-1.5" onClick={() => reconectar(i.id)}>
