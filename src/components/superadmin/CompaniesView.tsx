    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] text-gray-400 mt-0.5">
            {kpis.total} negocios en plataforma
          </p>
        </div>
        <Button
          size="sm"
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/5">
              {[
                ...
                <th
                  key={h}
                  className="px-4 py-3 text-left text-[10px] font-black uppercase tracking-widest text-gray-500"
                >
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {top5.map((c, i) => (
              ...
              >
                <td className="px-4 py-3.5 text-gray-500 font-bold text-sm">
                  {i + 1}
                </td>
                <td className="px-4 py-3.5 font-bold text-white/90 group-hover:text-white transition-colors">
                  {c.name}
                </td>
                ...
                <td className="px-4 py-3.5">
                  <BoolIcon value={c.hasPayment} />
                </td>
                <td className="px-4 py-3.5">
                  <BoolIcon value={c.hasSunat} />
                </td>
                <td className="px-4 py-3.5 text-gray-300 font-mono text-xs">
                  {c.waMessages?.toLocaleString() || "—"}
                </td>
                <td className="px-4 py-3.5 text-gray-300 font-mono text-xs">
                  {c.shipments?.toLocaleString() || "—"}
                </td>
                <td className="px-4 py-3.5">
                  <ChurnBadge risk={c.churnRisk || "BAJO"} />
                </td>
                <td className="px-4 py-3.5">
      ...
      <div className="bg-[#1a1f2e] border border-white/5 rounded-2xl overflow-hidden">
        <div className="flex flex-wrap items-center gap-3 px-5 py-4 border-b border-white/5">
          <h3 className="text-sm font-black text-white mr-auto">
            Todas las Empresas
          </h3>
            ...
            <thead>
              <tr className="border-b border-white/5">
                {[
                  ...
                  <th
                    key={h}
                    className="px-5 py-3 text-left text-[10px] font-black uppercase tracking-widest text-gray-500"
                  >
                    {h}
                  </th>
                ))}
                ...
                >
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1.5 text-xs text-gray-400">
                      ...
                      {c.userId
                        ? allUsers.find((u: any) => u.id === c.userId)
                            ?.lastSignInAt
                          ? formatDistanceToNow(
                              parseISO(
                                allUsers.find((u: any) => u.id === c.userId)
...
function StatusBadge({ status }: { status: string }) {
  if (status === "ACTIVE")
    return (
      <span className="text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-400">
        Activo
      </span>
    );
  ...
  if (status === "EXPIRED")
    return (
      <span className="text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded bg-rose-500/20 text-rose-400">
        Vencido
      </span>
    );
  if (status === "PENDING_PAYMENT")
    return (
      <span className="text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded bg-sky-500/20 text-sky-400">
        Pend. Pago
      </span>
    );
  return (
    <span className="text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded bg-gray-500/20 text-gray-400">
      {status || "N/A"}
    </span>
  );
}
...
function CompanyDetailModal({
  ...
  const fetchDetails = useCallback(async () => {
    ...
    try {
      ...
      const { getCompanySalesSummary } =
        await import("@/services/salesService");
      ...
      const [users, productCount, sales] = await Promise.all([
        ...
        getCompanySalesSummary(company.id, fromStr, toStr).catch(() => ({
          totalSales: 0,
          orderCount: 0,
          income: [],
        })),
      ...
      setDetails((p: any) => ({ ...p, loading: false }));
    }
  }, [company, date]);
  ...
  const handlePlanChange = async (planId: string) => {
    ...
    try {
      ...
      if (active) {
        await updateSubscription(active.id, { planId });
        toast.success("Plan actualizado correctamente");
      } else {
        await createSubscription({
          userId: company.userId,
          planId,
          payerEmail: company.billingEmail || "support@powip.com",
        });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className="w-[96vw] sm:max-w-4xl max-h-[90vh] overflow-y-auto bg-[#12151f] border border-white/10 text-white p-0"
      >
        {/* Header */}
        <div className="flex items-start justify-between px-6 pt-6 pb-5 border-b border-white/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <DialogDescription className="text-[11px] text-gray-500 font-mono mt-0.5">
                ID: {company?.id}
              </DialogDescription>
            </div>
          </div>
        ...
        {details.loading ? (
          ...
          <div className="px-6 py-5 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-end sm:items-center justify-end mb-4 gap-2">
              <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mr-2">
                Filtrar por fecha
              </Label>
              <DateRangePicker
                date={date}
            ...
            <div className="grid grid-cols-4 gap-3">
              {[
                ...
                {
                  label: "Ventas (30d)",
                  value: details.sales.orderCount,
                  icon: TrendingUp,
                  sub: details.trend,
                  trendUp: !details.trend?.startsWith("-"),
                },
                {
                  label: "Facturado",
                  value: `S/ ${details.sales.totalSales?.toLocaleString()}`,
                  icon: CreditCard,
                  accent: true,
                },
              ].map(
                ({ label, value, icon: Icon, accent, sub, trendUp }: any) => (
                  <div
                    key={label}
                    className="bg-white/5 border border-white/5 rounded-xl p-4 flex flex-col gap-2 overflow-hidden"
                  >
                    <div className="flex items-center justify-between">
            ...
            <div className="bg-white/5 border border-white/5 rounded-xl p-5 space-y-4">
              ...
              <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-white/5">
                ...
                <Button
                  ...
                  disabled={deleting}
                  className="h-9 text-rose-400 hover:bg-rose-500/10 hover:text-rose-300 border border-rose-500/20 text-[11px] font-bold uppercase tracking-wider"
                >
                  ...
                  <thead>
                    <tr className="border-b border-white/5">
                      {["Nombre", "Email", "Rol", "Estado"].map((h) => (
                        <th
                          key={h}
                          className="px-4 py-2.5 text-left text-[10px] font-black uppercase tracking-widest text-gray-600"
                        >
                          {h}
                        </th>
                      ))}
                    ...
                    {details.users.map((u: any) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <span className="text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded bg-primary/20 text-primary">
                            {u.role?.name || "User"}
                          </span>
                        </td>
                        <td className="px-4 py-3">
            ...
            <div className="flex justify-end pt-2 border-t border-white/5">
              <Button
                variant="ghost"
                onClick={() => onOpenChange(false)}
                className="h-9 text-gray-400 hover:text-white text-sm"
              >
                Cerrar
              </Button>
            </div>
...
function CreateCompanyModal({
  ...
  const handleSave = async () => {
    ...
    if (!owner) {
      toast.error("No se encontró ese email de usuario");
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent
        showCloseButton={false}
        className="sm:max-w-lg bg-[#12151f] border border-white/10 text-white p-0"
      >
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <DialogDescription className="text-[11px] text-gray-500">
                Registrar un negocio en la plataforma
              </DialogDescription>
            </div>
          </div>
          <button
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          {field(
            "ownerEmail",
            "Email del dueño (usuario existente)",
            "admin@empresa.com",
            "email",
          )}
        ...
        <div className="flex items-center justify-end gap-3 px-6 pb-6 pt-2 border-t border-white/5">
          ...
          <Button
            onClick={handleSave}
            disabled={loading}
            className="h-9 bg-primary hover:bg-primary/90 text-white text-sm font-bold shadow-lg shadow-primary/20 min-w-[140px]"
          >
            {loading ? (
              <>
