...
import { useKpisSuscripciones } from "@/hooks/superadmin/useSuscripciones";
...
export function SuscripcionesKpis() {
  const { data, isLoading } = useKpisSuscripciones();
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-6">
      ...
      <KpiCard icon={CheckCircle2} color="green" label="Activas" value={data.activas} simulado={data.isEstadoSimulado} />
      <KpiCard icon={Sparkles} color="blue" label="Nuevas (30d)" value={data.nuevas30d} simulado={data.isEstadoSimulado} />
      <KpiCard icon={TrendingDown} color="red" label="Canceladas" value={data.canceladas} simulado={data.isEstadoSimulado} />
