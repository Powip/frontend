...
import { useMrrPorPlan } from "@/hooks/superadmin/useSuscripciones";
...
export function MrrPorPlanCard() {
  const { data, isSimulado } = useMrrPorPlan();

  const total = data.data.reduce((sum, d) => sum + d.mrr, 0);
    ...
    <Card className="shadow-sm h-full">
      ...
      <CardContent>
        <Donut
          ...
          data={data.data.map((d) => ({ label: d.plan, value: d.mrr, pct: d.pct, color: COLORS[d.plan] ?? "#94A3B8" }))}
        />
      </CardContent>
    </Card>
  );
